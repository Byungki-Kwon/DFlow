...
def train(args):
    ...
    teacher_dir = "/home/kwon/eccv/eccv_dupdate/model/manual_remap_modi/500_raft-kitti-noaug.pth"
    print(teacher_dir)
    ...
    while should_keep_training:
        for i_batch, data_blob in enumerate(zip(back_loader, fore_loader)):
            ...
            do_color_change = False

            for k in range(130):

                ll = (2 - (k/80)) * 2

                # Backward-warping1 -> next : affine-warping(translation[2], rotation[1], zooming[2]), grid-warping[8x12x2]
                # Backward-warping2 -> prev : affine-warping(translation[2], rotation[1], zooming[2]), grid-warping[8x12x2], perspective_warping[8]
                # Foreground_offset -> prev,flow : translation[(n-1)*2]
                # Texture_manipulator-> 공통: (0~1)값을 가짐 크기는 n*3
                # Texture_noise -> prev_noise[HxWx3], next_noise[HxWx3]
                # Fog -> 공통 : Fog color [4x4x3]
                # Motion_blur -> Gaussian blur kernel sigma x [1], Gaussian blur kernel sigma y [1], Gaussian blur kernel sigma rotation [1]

                optimizer = optim.Adam(
                    [
                        {"params": motion_init, "lr": 2 * ll},# 2
                        {"params": zoom_init, "lr": 0.002 * ll},# 2
                        {"params": rot_init, "lr": 0.01 * ll},# 1
                        {"params": motion_next, "lr": 0.1 * ll},# 2
                        {"params": zoom_next, "lr": 0.002 * ll},# 2
                        {"params": rot_next, "lr": 0.01 * ll},# 1
                        {"params": noise1, "lr": 0.01 * ll},# HxWx3
                        {"params": noise2, "lr": 0.01 * ll},# HxWx3
                        {"params": dst_points, 'lr': 0.1 * ll},# 8
                        {"params": grid_warping1, 'lr': 0.5 * ll},# 8*12*2 = 192
                        {"params": grid_warping2, 'lr': 0.5 * ll },# 8*12*2 = 192
                        {"params": fore_offset, 'lr': 0.2 * ll},# (n-1)*2
                        {"params": k_sigma_x_bs, 'lr': 0.01 * ll},# 1
                        {"params": k_sigma_y_bs, 'lr': 0.01 * ll},# 1
                        {"params": fog_color, 'lr': 0.01 * ll},# 4x4x3
                        {"params": k_angle_bs, 'lr': 0.01 * ll},# 1
                        {"params": color_change, 'lr': 0.01 * ll}# n*3
                    ],
                    lr=0.0001
                )
